...
import { addRecord, deleteRecord, formatRecordError, listRecords, toNumber, updateRecord, validateAmount, type FinancialRecord } from '@/lib/financial-data';
...
export default function ExpensesPage() {
  ...
  async function saveExpense() {
    ...
    try {
      const payload = { category, amount: Number(amount), expense_type: expenseType, record_date: recordDate, description: description.trim() || null };
      const record = editingId ? await updateRecord('expense_records', editingId, payload) : await addRecord('expense_records', payload);
      setExpenses((current) => editingId ? current.map((item) => item.id === editingId ? record : item) : [record, ...current]); setAmount(''); setDescription(''); setEditingId(null);
